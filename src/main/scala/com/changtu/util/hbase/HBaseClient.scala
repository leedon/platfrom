...
import org.apache.hadoop.hbase.client.{ConnectionFactory, Delete, Get, Put, Result, Scan, Table}
...
class HBaseClient(config: Configuration = HBaseConfiguration.create(), tablePath: String,
                  limit: Option[Int] = None, verboseMode: Boolean = false, caching: Int = 5000) extends Logging {
  ...
  logger.info("Creating connection now!")
  ...
  logger.info("Creating connection successful!")
  ...
  def printVerboseQueryInfo(scan: Scan): Unit = {
    ...
    logger.info("Scanning table: " + tablePath)
    logger.info("FamilyMap: " + map)
    logger.info("Start row: " + Bytes.toString(scan.getStartRow))
    logger.info("Stop row: " + Bytes.toString(scan.getStopRow))
    logger.info("Filter: " + scan.getFilter.toString)
  }
  ...
  def printVerboseQueryInfo(get: Get): Unit = {
    ...
    logger.info("Getting row from table: " + tablePath)
    logger.info("Row key: " + Bytes.toString(get.getRow))
    logger.info("FamilyMap: " + map)
    logger.info("Filter: " + get.getFilter)
  }
  ...
  def deleteRows(filters: FilterList, f: (Result => Boolean)) {
    logger.info("Delete operation using filters " + filters)

    def r = (res: Result, htable: Table) => {
      if (f(res)) {
        val row = res.getRow
        htable.delete(new Delete(row))
        logger.info("Deleted row: " + Bytes.toString(row))
      }
    }
    val scan = getScan
  ...
  def execute(scan: Scan, r: ((Result, Table) => Any)) {
    ...
    val resultFunction = if (verboseMode) {
      ...
      (res: Result, table: Table) => {
        logger.info("Result row: " + Bytes.toString(res.getRow))
        r(res, table)
      }
        logger.info("Scan completed for " + tablePath + " with filter " + scan.getFilter)
    } else r
    ...
    if (results != null) {
      results.foreach { res =>
        ...
        i += 1
        if (i % bigN == 0) {
          logger.info("Scanning more than " + i + " rows from " + tablePath + " with filter " + scan.getFilter)
        }
        resultFunction(res, htable)
      }
      if (i > bigN) {
        logger.info("Scanned " + i + " rows from " + tablePath + " with filter " + scan.getFilter)
      }
    }
    scanner.close()
  }
