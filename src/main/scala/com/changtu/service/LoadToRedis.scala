...
/**
  ...
  */
object RedisClient {
  val client = new RedisUtils("redis.server.1")
}
...
object LoadToRedis {
  ...
  def main(args: Array[String]) {
    ...
    val srcFileRDD = sc.textFile(srcFiles).filter(!_.isEmpty)
      .map(p => p.split(fieldTerminate))
      .map(p => (p(fieldPosition.toInt), p(valuePosition.toInt)))

    srcFileRDD.foreachPartition(part => {
      val data = part.toMap
      RedisClient.client.hmset(key, data)
    })
  }
